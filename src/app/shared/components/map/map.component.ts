...
export class MapComponent implements OnChanges {
  ...
  public optionsSpec: {
    center: LatLngTuple;
    layers: { attribution: string; url: string }[];
    zoom: number;
  } = {
    layers: [
      {
  ...
  public options: { layers: TileLayer[]; zoom: number; center: LatLng } = {
    layers: [
      tileLayer(this.optionsSpec.layers.at(0)!.url, {
        attribution: this.optionsSpec.layers.at(0)!.attribution,
      }),
    ],
  ...
  private addMarker(): void {
    const newMarker = marker(this.markerOptions.position, {
      icon: icon({
        iconSize: [50, 50],
        iconAnchor: [13, 41],
